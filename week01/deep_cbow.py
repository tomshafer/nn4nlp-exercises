...
def cli() -> ap.ArgumentParser:
    ...
    p.add_argument("--hidden", type=int, default=64, dest="HIDDEN_SIZE")
...
if __name__ == "__main__":
    ...
    train_l = DataLoader(
        DATA.train, batch_size=64, num_workers=2, collate_fn=pad_collate
    )
    dev_l = DataLoader(DATA.dev, batch_size=64, num_workers=2, collate_fn=pad_collate)
    ...
    torch.save(
        model, f"deepcbow_model_e{args.EPOCHS:04d}_d{dropout_str}_s{args.EMBEDDING_SIZE}_h{args.HIDDEN_SIZE}.pt"
    )
    torch.save(
        histories,
        f"deepcbow_history_e{args.EPOCHS:04d}_d{dropout_str}_s{args.EMBEDDING_SIZE}_h{args.HIDDEN_SIZE}.pt",
    )
